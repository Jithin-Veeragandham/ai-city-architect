...
def load_images(cell_size):
    ...
    images = {
        1: Image.open(rf"D:\NEU MS CS\FAI\Project\ai-city-architect\res\building.jpg").resize(
            (cell_size, cell_size)
        ),  # Building image (JPEG format)
        2: Image.open(rf"D:\NEU MS CS\FAI\Project\ai-city-architect\res\emergency.png").resize(
            (cell_size, cell_size)
...
def save_city_grid(grid, paths, dir_name, output_file_name):
    ...
    for y in range(grid_size):
        for x in range(grid_size):
            ...
            if grid[y][x] == 3:  # Intersection
                ...
                draw.rectangle(
                    [inner_top_left, inner_bottom_right], fill=road_color)
    ...
    for i, path in enumerate(paths):
        ...
        for (coord, direction) in path:
            ...
            path_top_left = (x * CELL_SIZE + offset_x,
